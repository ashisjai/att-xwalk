...
export default function decorate(block) {
  ...
  [...block.children].forEach((row) => {
    ...
    // Columns: image, eyebrow, heading, subHeading, description, legal, cta, secondaryCta
    const cols = [...row.children];
    const [imageCol, eyebrowCol, headingCol, subHeadingCol, descCol,
      legalCol, ctaCol, secondaryCol] = cols;
    ...
    if (subHeadingCol?.textContent.trim()) {
      const subHeading = document.createElement('p');
      subHeading.className = 'sub-heading';
      subHeading.textContent = subHeadingCol.textContent.trim();
      bodyDiv.append(subHeading);
    }
